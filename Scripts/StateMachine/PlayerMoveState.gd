...
func DetectStateChange():
	var isInputting = Input.get_vector("left", "right", "up", "down") != Vector2.ZERO
	var justPressedJump = Input.is_action_just_pressed("jump")
	var isMoving = abs(player.velocity.x) > 0.01 and abs(player.velocity.y) > 0.01 and abs(player.velocity.y) > 0.01
	if not isInputting and not justPressedJump and not isMoving:
		stateTransition.emit(self, "PlayerIdleState")
